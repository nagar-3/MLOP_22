...
gamma = [0.001,0.001,0.0005,0.0001]
...
for i,j in zip(gamma,c_value):
  for j in c_value:
    ...
    print("[ Gamma: {0: <6}, C: {1: <6}  ==> Train Accuracy : {2: <6}  Dev Accuracy : {3:<6}  Test Accuracy : {4: <6} ".format(i,j,acc_train,acc_val,acc_test))
...
print(val_test_acc)
print("\n----------------------------------------------------------------")
print("Best Parms Gamma: {0:<6} , C: {1:<6}".format(best_prms["Gamma"],best_prms["C"]))
print("\n------------------------- train Report ---------------------------")
print(Report_train)
print("\n------------------------- dev Report ---------------------------")
print(Report_val)
print("\n------------------------- Test Report --------------------------")
print(Report_test)
print("Train data   Mean {0: <10}   Median : {1: <8}   Mode {2: <8}".format(str(statistics.mean(acc_traint)),statistics.median(acc_traint),statistics.mode(acc_traint)))
print("Dev Data     Mean {0: <10}   Median : {1: <8}   Mode {2: <8}".format(str(statistics.mean(acc_valt)),statistics.median(acc_valt),statistics.mode(acc_valt)))
print("Test Data    Mean {0: <10}   Median : {1: <8}   Mode {2: <8}".format(str(statistics.mean(acc_testt)),statistics.median(acc_testt),statistics.mode(acc_testt)))
print("\n")
...
def test_Q_2():
    assert len(set(predicted3))>1
    
def test_Q_3():
    assert len(set(predicted3))==len(set(y_train))
